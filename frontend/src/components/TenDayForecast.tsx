...
export function TenDayForecast({ weather }: TenDayForecastProps) {
  ...
  return (
    <section className="rounded-2xl border border-theme bg-surface backdrop-blur-xl">
      <header className="flex items-center gap-2 border-b border-theme-soft px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-theme-muted">
        <CalendarIcon />
        ...
        {days.length > 0 ? (
          days.map((day, index) => {
            ...
            return (
              ...
              >
                <span className="font-medium text-theme">{labelForDate(day.date, index)}</span>
                <CloudIcon className="h-5 w-5 text-theme-muted" />
                <span className="tabular-nums text-theme-muted">
                  {formatTemperature(day.temperature_low_c)}
                </span>
                <div className="relative h-1.5 rounded-full" style={{ background: 'var(--border-soft)' }}>
                  {offsets && (
                    <div
                ...
                <span className="text-right tabular-nums text-theme">
                  {formatTemperature(day.temperature_high_c)}
                </span>
                {day.forecast && (
                  <span className="col-span-5 -mt-1 truncate text-xs text-theme-muted">
                    {day.forecast}
                  </span>
                )}
              </li>
          ...
          <li className="px-4 py-4 text-sm text-theme-faint">Forecast unavailable.</li>
        )}
      </ul>
    </section>
  );
