...
function TileShell({ icon, title, className = '', children }: TileShellProps) {
  return (
    <section
      className={`flex flex-col gap-3 rounded-2xl border border-theme bg-surface p-4 backdrop-blur-xl ${className}`}
    >
...
export function AirQualityTile({ weather }: WeatherProps) {
  ...
  return (
    ...
    >
      <div className="text-4xl font-light leading-none text-theme">{psi}</div>
...
export function AveragesTile({ weather }: WeatherProps) {
  return (
    <TileShell icon={<TrendIcon />} title="Forecast High">
      ...
      <div className="mt-1 text-xs leading-snug text-theme-muted">
        Today&apos;s high from the 24-hour forecast.
      </div>
      <ul className="mt-3 space-y-1 text-xs text-theme-muted">
        <li
